...
//        case timerResetRecomendation([ReasonsToStop], UserSmokingSessionMetrics)
    ...
    private var typeOfGenerationEvent: TypeOfRecomendation = .moodRecomendation
...
//    }
    //MARK: init
    init(user: User, currentUser: User/*, storageService: FirebaseStorageServiceProtocol*/) {
        self.userData = user
...
//        getUserData()
    ...
    func generateResponse() async {
        ...
        print("CURRENT USER DATA \(currentUser.name)")
...
//        let query: ChatQuery
        ...
        let query = ChatQuery(messages: [.init(role: .user, content: RecomendationPrompts.getRecomendationForMoodAdded(userData: userData, currentUser: currentUser))!], model: .gpt4)
        
        print(query)
//        let query = ChatQuery(messages: [.init(role: .user, content: "Recomend me a movie based on my mood. My current mood is: \(prompt)"), .init(role: .system, content: "RESPOND WITH ONLY ONE IMBD ID")], model: .gpt4)
//        
//        query = ChatQuery(model: .gpt3_5Turbo, messages: [.init(role: .user, content: RecomendationPrompts.getRecomendationForMoodAdded(userData: userData, currentUser: currentUser))], maxTokens: tokens)
        do {
            let result = try await openAi.chats(query: query)
            recomendation = (result.choices[0].message.content?.string!)!
            state = .loaded
        } catch {
