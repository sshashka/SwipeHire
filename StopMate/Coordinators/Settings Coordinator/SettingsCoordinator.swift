...
//        storageService.retrieveUserProfilePic(userEmail: <#T##String#>)
    }
    ...
    func showSettings() {
        ...
        vm.didSendEventClosure = { [weak self] event in
            ...
            case .didTapOnEdit:
                self?.didTapOnEdit()
            case .didTapOnOnboarding:
                self?.didTapOnOnboarding()
            }
        }
...
//        navigationController.pushViewController(vc, animated: true)
    ...
    private func didTapOnOnboarding() {
        self.replaceWithNewCoordinator(coordinator: .onboarding)
    }
